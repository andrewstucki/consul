...
func (s *Restore) KVS(entry *structs.DirEntry) error {
	if err := insertKVTxn(tableKVs, s.tx, entry, true, true); err != nil {
...
func (s *Store) KVSSet(idx uint64, entry *structs.DirEntry) error {
	...
	if err := kvsSetTxn(tableKVs, tx, idx, entry, false); err != nil {
		return err
	}
...
func (s *Store) KVSGet(ws memdb.WatchSet, key string, entMeta *acl.EnterpriseMeta) (uint64, *structs.DirEntry, error) {
	...
	return kvsGetTxn(tableKVs, tableTombstones, tx, ws, key, *entMeta)
}
...
func (s *Store) KVSList(ws memdb.WatchSet,
	...
	return s.kvsListTxn(tableKVs, tableTombstones, s.kvsGraveyard, tx, ws, prefix, *entMeta)
}
...
func (s *Store) KVSDelete(idx uint64, key string, entMeta *acl.EnterpriseMeta) error {
	...
	if err := s.kvsDeleteTxn(tableKVs, s.kvsGraveyard, tx, idx, key, entMeta); err != nil {
		return err
	}
...
func (s *Store) KVSDeleteCAS(idx, cidx uint64, key string, entMeta *acl.EnterpriseMeta) (bool, error) {
	...
	set, err := s.kvsDeleteCASTxn(tableKVs, s.kvsGraveyard, tx, idx, cidx, key, entMeta)
	if !set || err != nil {
		return false, err
	}
...
func (s *Store) KVSSetCAS(idx uint64, entry *structs.DirEntry) (bool, error) {
	...
	set, err := kvsSetCASTxn(tableKVs, tx, idx, entry)
	if !set || err != nil {
		return false, err
	}
...
func (s *Store) KVSDeleteTree(idx uint64, prefix string, entMeta *acl.EnterpriseMeta) error {
	...
	if err := s.kvsDeleteTreeTxn(tableKVs, s.kvsGraveyard, tx, idx, prefix, entMeta); err != nil {
		return err
	}
...
func (s *Store) KVSLock(idx uint64, entry *structs.DirEntry) (bool, error) {
	...
	locked, err := kvsLockTxn(tableKVs, tx, idx, entry)
	if !locked || err != nil {
		return false, err
	}
...
func (s *Store) KVSUnlock(idx uint64, entry *structs.DirEntry) (bool, error) {
	...
	unlocked, err := kvsUnlockTxn(tableKVs, tx, idx, entry)
	if !unlocked || err != nil {
		return false, err
	}
